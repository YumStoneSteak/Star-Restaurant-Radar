...
1. GitHub Releases에서 `StarRestaurantRadar-Setup-v2.0.0.exe`를 다운로드합니다.
...
현재 설치된 버전보다 새 버전이 있으면 설치 파일과 `.sha256` 파일을 백그라운드에서 다운로드합니다. 무결성 검사를 통과한 설치 파일만 자동으로 실행하며, 설치가 끝나면 앱이 다시 실행됩니다. 확인과 다운로드 중에도 설정창은 계속 응답합니다. 업데이트 확인은 사용자가 직접 눌렀을 때만 실행됩니다.
...
- `dist\StarRestaurantRadar-Setup-v2.0.0.exe` (설치 파일)
- `dist\StarRestaurantRadar-Setup-v2.0.0.exe.sha256`
